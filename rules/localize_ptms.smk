ALL_LOCAL = itertools.chain.from_iterable([
              expand("results/localization/{dataset}/{sample}.pyascore.{type}.txt",
                     dataset = d,
...
rule pyascore:
    input:
        ...
        ids = "results/localization/{dataset}/{sample}.mokapot.psms.txt",
        params = lambda w: SAMPLES_BY_DATASET[w.dataset]["pyascore_params"]
    output:
        "results/localization/{dataset}/{sample}.pyascore.{type}.txt"
    conda:
        SNAKEMAKE_DIR + "/envs/pyascore.yaml"
    params:
        mz_error = lambda w: {"wide" : .5, "narrow" : .05}[w.type],
    shell:
        """
